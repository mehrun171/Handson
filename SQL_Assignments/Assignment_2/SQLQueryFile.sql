...
WHERE deptno in (select deptno from emp where deptno in 
...
select job,COUNT(job) as 'No_Of_Clerks_Employed' 
...
COUNT(job) as 'No_Of_People' from emp
group by job
...
select mgr_id,count(mgr_id) as 'Reporting_Employees' from emp 
